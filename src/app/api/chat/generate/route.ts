...
export async function POST(req: Request) {
  ...
  const session = await db.execute({ sql: `SELECT id FROM chat_sessions WHERE id = ? AND owner_google_sub = ? LIMIT 1`, args: [body.sessionId, user.sub] });
  if (!session.rows[0]) return NextResponse.json({ error: "session_not_found" }, { status: 404 });

  let templatePrompt = "";
  if (body.promptTemplateId) {
    const tpl = await db.execute({ sql: `SELECT prompt_text FROM prompt_templates WHERE id = ? AND is_active = 1 LIMIT 1`, args: [body.promptTemplateId] });
  ...
  if (body.selectedAssetIds.length > 0) {
    ...
    const res = await db.execute({ sql: `SELECT id, blob_url FROM assets WHERE owner_google_sub = ? AND id IN (${placeholders})`, args: [user.sub, ...body.selectedAssetIds] });
  ...
  try {
    ...
    await db.execute({ sql: `INSERT INTO generation_events (id, owner_google_sub, session_id, message_id, status, error_code, error_message, created_at) VALUES (?, ?, ?, ?, 'failed', ?, ?, ?)`, args: [newId("evt"), user.sub, body.sessionId, userMessageId, code, "Generation failed", nowIso()] });
